...
def plot_surface_code_heatmap_layout(
    ...
    """
    Draw the rotated surface code layout with a heatmap colouring
    of ancilla plaquettes.
    ...
    # ---- 4) ancilla markers (outline only) ----
    for q, (x, y) in anc_pts.items():
        ax.scatter(
            [x], [y],
            marker="o",
    ...
    ax.set_xlabel("X coordinate of qubits", fontsize=16)
    ax.set_ylabel("Y coordinate of qubits", fontsize=16)
    ax.tick_params(axis="both", which="major", labelsize=16)
    ax.grid(True, linestyle=":", color="#DDDDDD")
    ax.invert_yaxis()
    plt.tight_layout()

    # add colorbar (use the list of vals as a ScalarMappable)
    import matplotlib as mpl
    norm = mpl.colors.Normalize(vmin=vmin, vmax=vmax)
    sm = mpl.cm.ScalarMappable(norm=norm, cmap=cmap)
    sm.set_array(vals)
    cbar = plt.colorbar(sm, ax=ax, fraction=0.046, pad=0.05)
    # cbar.set_label("P(syndrome = 1)")
    plt.subplots_adjust(right=0.85)
    print("Saving figure to:", os.path.abspath("bad9.png"))
    plt.savefig("bad9.svg", bbox_inches="tight")
    plt.savefig("bad9.png", dpi=300, bbox_inches="tight")
    plt.show()



def visualize_syndrome_heatmap_across_all_rounds(
    circuit: stim.Circuit,
    distance: int,
    num_round: int,
    ...
    """
    final_data: shape (num_shots, num_round * num_ancilla)

    This version:
      - reshapes to (shots, rounds, ancilla)
      - averages over *shots and rounds*
      - plots a single heatmap layout.
    """
    num_shots = final_data.shape[0]
    num_ancilla = distance**2 - 1

    # [shots, rounds, ancilla]
    ancilla_bits = final_data.reshape(num_shots, num_round, num_ancilla)

    # average over shots AND rounds: P(syndrome = 1) per ancilla
    mean_per_ancilla = ancilla_bits.mean(axis=(0, 1))   # <--- key line

    # map ancilla index -> ancilla qubit id
    data_qubits, ancilla_qubits = classify_qubits(circuit)
...
def main():
    ...
    # Noise parameters
    p1 = 0   # single qubit gate depolarizing
    p2 = 0   # two qubit gate depolarizing
    pRM = 0 # reset/measurement flip probability

    #Enter the round values you want to generate data for
    round_values = list(range(20, 22,2 ))
    ...
    bad_qubit = 9
    factor = 10.0
    # dir = "C:/Research_Chaitali/phy191a/Error_test/test"
    dir = "C:/Research_Chaitali/phy191a/Bad_qubit_9_test/test"
    for num_round in round_values:
        filename = os.path.join(dir, f"d{distance}_r{num_round}_b{bad_qubit}_f{factor}_test.h5")
        # filename = os.path.join(dir, f"d{distance}_r{num_round}_test.h5")
        if not os.path.isfile(filename):
            print(f"WARNING: file not found: {filename} – skipping")
            continue

        with h5py.File(filename, "r") as hf:
            # Load circuit string (stored as bytes)
            circuit_str = hf["circuit"][()].astype(str)    
            # Reconstruct stim.Circuit
            circuit = stim.Circuit(circuit_str)
            syndromes = hf["syndromes"][()]     # numpy array
            detectors = hf["detectors"][()]     # numpy array

# # ONE heatmap: average over all shots and all rounds
#         visualize_syndrome_heatmap_across_all_rounds(
#             circuit=circuit,
#             distance=distance,
#             num_round=num_round,
#             final_data=syndromes,
#             cmap_name="Reds",
#         )

                # visualize ONLY round 20 (index 19)
        visualize_syndrome_heatmap_for_round(
            circuit=circuit,
            distance=distance,
            num_round=20,
            final_data=syndromes,
            round_idx=num_round-1 ,  # 19
            cmap_name="Reds",
        )
